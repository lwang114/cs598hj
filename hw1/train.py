...
from model_multilingual import MultilingualLstmFet
from model_attention import AttentionLstmFet
...
parser.add_argument('--device', choices={'gpu', 'cuda:0', 'cuda:1'}, default='cuda:0', help='Use CPU or GPU')
parser.add_argument('--embedding_type', choices={'cbow', 'multilingual-cbow', 'combined-cbow'}, default='cbow', help='Embedding type')
...
parser.add_argument('--model_type', choices={'lstm', 'lstm_joint_embed', 'lstm_attention'}, default='lstm', help='Model architecture used')
...
gpu = (args.device == 'cuda:0') or (args.device == 'cuda:1')
...
elif args.embedding_type == 'combined-cbow':
    embed_file = '{}/wiki.multi.en.vec:{}/cc.en.300.vec'.format(root, root)
...
# Datasets
train_set = FetDataset(train_file, device=args.device)
dev_set = FetDataset(dev_file, device=args.device)
test_set = FetDataset(test_file, device=args.device)
...
if args.model_type == 'lstm':
  if args.embedding_type == 'combined-cbow':
      combine_linear = torch.nn.Linear(embed_dim * 2, embed_dim)
      linear = torch.nn.Linear(embed_dim * 2, label_num)
      lstm = torch.nn.LSTM(embed_dim, embed_dim, batch_first=True)
      model = MultilingualLstmFet(word_embed, lstm, combine_linear, linear, embed_dropout, lstm_dropout)
  else:
      linear = torch.nn.Linear(embed_dim * 2, label_num)
      lstm = torch.nn.LSTM(embed_dim, embed_dim, batch_first=True)
  ...
  if gpu:
      label_embed_matrix = label_embed_matrix.to(device=args.device)
  ...
  output_linear = torch.nn.Linear(embed_dim * 2, label_num)
  attend_layer = torch.nn.Sequential(torch.nn.Linear(embed_dim*2, embed_dim),
                                     torch.nn.Tanh(),
                                     torch.nn.Linear(embed_dim, 1))
  lstm = torch.nn.LSTM(embed_dim, embed_dim, batch_first=True)
  model = AttentionLstmFet(word_embed, lstm, attend_layer, output_linear, embed_dropout, lstm_dropout)
