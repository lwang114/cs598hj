...
class BiLSTM_Joint(nn.Module):
	def __init__(self, config):
		super(BiLSTM_Joint, self).__init__()
		self.config = config
		...
		self.linear_ne = nn.Linear(hidden_size*2, config.entity_type_size)
	...
	def forward(self, context_idxs, pos, context_ner, context_char_idxs, context_lens, h_mapping, t_mapping,
		...
		predict_ne = self.linear_ne(context_output)
