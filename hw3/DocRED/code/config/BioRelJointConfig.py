...
class BioRelJointConfig(object):
	def __init__(self, args):
		self.acc_NA = Accuracy()
		self.acc_not_NA = Accuracy()
		self.acc_total = Accuracy()
		self.acc_ne = Accuracy()
	...
	def load_test_data(self):
		...
		self.ner2id = json.load(open(os.path.join(self.data_path, 'ner2id.json')))
	...
	def get_test_batch(self):
		...
		e_mapping = torch.Tensor(self.test_batch_size, self.entity_num, self.max_length).cuda()
		...
		for b in range(self.test_batches):
			...
			for i, index in enumerate(cur_batch):
				...
				j = 0
				for e_idx in range(L):
					elist_by_names = ins['entities'][e_idx]['names'].values()
					elist = [e_mention for e_info in elist_by_names for e_mention in e_info['mentions'] if e_info['is_mentioned']]
					for e in elist:
						e[0] = inv_span[int(e[0])]
						e[1] = inv_span[int(e[1]-1)]
						if e[0] == e[1]:
							e[1] += 1
						e_mapping[e_idx, e[0]:e[1]] = 1. / len(elist) / (e[1] - e[0])
			...
			yield {'context_idxs': context_idxs[:cur_bsz, :max_c_len].contiguous(),
				   ...
				   'h_mapping': h_mapping[:cur_bsz, :max_h_t_cnt, :max_c_len],
				   't_mapping': t_mapping[:cur_bsz, :max_h_t_cnt, :max_c_len],
					 'e_mapping': e_mapping[:cur_bsz, :max_c_len, :max_c_len],
				   'labels': labels,
				   'L_vertex': L_vertex,
				   'input_lengths': input_lengths,
				   'context_ner': context_ner[:cur_bsz, :max_c_len].contiguous(),
	...
	def train(self, model_pattern, model_name):
		...
		CE = nn.CrossEntropyLoss(size_average=True, ignore_index=0)
		...
		for epoch in range(self.max_epoch):
			...
			for ex, data in enumerate(self.get_train_batch()):
				...
				predict_re, predict_ne = model(context_idxs, context_pos, context_ner, context_char_idxs, input_lengths, h_mapping, t_mapping, relation_mask, dis_h_2_t, dis_t_2_h)
				loss = torch.sum(BCE(predict_re, relation_multi_label)*relation_mask.unsqueeze(2)) /  (self.relation_num * torch.sum(relation_mask)) 
				loss += torch.sum(CE(predict_ne.view(-1, self.entity_type_size), context_ner.flatten()) / self.entity_type_size)

				output = torch.argmax(predict_re, dim=-1)
				output = output.data.cpu().numpy()
				output_ne = torch.argmax(predict_ne, dim=-1)
				output_ne = output_ne.data.cpu().numpy()
				optimizer.zero_grad()
				loss.backward()
				optimizer.step()
				...
				for i in range(output_ne.shape[0]):
					for j in range(output_ne.shape[1]):
						label = context_ner[i][j]
						if label <= 0:
							continue
						self.acc_ne.add(output_ne[i][j] == label)
				...
				if global_step % self.period == 0 :
					...
					logging('| epoch {:2d} | step {:4d} |  ms/b {:5.2f} | train loss {:5.3f} | NA acc: {:4.2f} | not NA acc: {:4.2f}  |	tot acc: {:4.2f} | entity acc: {:4.2f} '.format(epoch, global_step, elapsed * 1000 / self.period, cur_loss, self.acc_NA.get(), self.acc_not_NA.get(), self.acc_total.get(), self.acc_ne.get()))
					total_loss = 0
					start_time = time.time()
	...
	def test(self, model, model_name, output=True, input_theta=-1):
		...
		for data in self.get_test_batch():
			with torch.no_grad():
				...
				predict_re, predict_ne = model(context_idxs, context_pos, context_ner, context_char_idxs, input_lengths,
								   h_mapping, t_mapping, relation_mask, dis_h_2_t, dis_t_2_h)

				predict_ne = torch.matmul(e_mapping, predict_ne)
				predict_re = torch.sigmoid(predict_re)

			predict_ne = predict_ne.data.cpu().numpy()
			predict_re = predict_re.data.cpu().numpy()
			for i in range(len(labels)):
				label = labels[i]
				index = indexes[i]

				for e_idx, e in enumerate(np.argmax(predict_ne[i], axis=-1).tolist()):
					test_result_ne.append((index, e_idx, self.id2ner[e]))
			...
			if data_idx % self.period == 0:
				...
				eval_start_time = time.time()
		json.dump(test_result_ne, open(os.path.join(self.fig_result_dir, model_name+'_test_entity_result.json'), 'w'), indent=4, sort_keys=True)
		...
		if output:
			...
			output_ne = [{'index': x[0], 'e_idx': x[1], 'e': x[2]} for x in test_result_ne]
			...
			json.dump(output_ne, open(pred_file+'_ne.json', 'w'), indent=4, sort_keys=True)
			self.convert_result(pred_file+'.json', pred_file+'_ne.json', pred_file+'_converted.json')
	...
	def convert_result(self, in_rel_file, in_ne_file, out_file):
		print(out_file)
		test_info = json.load(open(os.path.join(self.data_path, self.test_prefix+'.json')))
		predict_info = json.load(open(in_rel_file, 'r'))
		predict_ne_info = json.load(open(in_ne_file, 'r'))
		converted_pred_info = []
		for item in test_info:
			item['interactions'] = []
			converted_pred_info.append(item)
		
		for item in predict_ne_info:
			idx = item['index']
			e_idx = item['e_idx']
			if e_idx >= len(converted_pred_info[idx]['entities']):
				continue
			converted_pred_info[idx]['entities'][e_idx]['label'] = item['e'] 
