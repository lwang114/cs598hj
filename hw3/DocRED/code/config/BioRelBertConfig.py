...
class BioRelBertConfig(object):
	...
	def load_train_data(self):
		...
		self.data_train_wordpiece_map = np.load(os.path.join(self.data_path, prefix+'_wordpiece_map.npy'))
		self.data_train_wordpiece_mask = np.load(os.path.join(self.data_path, prefix+'_wordpiece_mask.npy'))
	...
	def load_test_data(self):
		...
		self.data_test_wordpiece_map = np.load(os.path.join(self.data_path, prefix+'_wordpiece_map.npy'))
		self.data_test_wordpiece_mask = np.load(os.path.join(self.data_path, prefix+'_wordpiece_mask.npy'))
	...
	def get_train_batch(self):
		...
		wordpiece_map = torch.LongTensor(self.batch_size, self.max_length, self.wordpiece_limit).cuda()
		wordpiece_mask = torch.FloatTensor(self.batch_size, self.max_length, self.wordpiece_limit).cuda()
		for b in range(self.train_batches):
			...
			for i, index in enumerate(cur_batch):
				...
				wordpiece_map[i].copy_(torch.from_numpy(self.data_train_wordpiece_map[index, :, :]))
				wordpiece_mask[i].copy_(torch.from_numpy(self.data_train_wordpiece_mask[index, :, :]))
			...
			data_dict = {'context_idxs': context_idxs[:cur_bsz, :max_c_len].contiguous(),
				     ...
				     'wordpiece_map': wordpiece_map[:cur_bsz, :max_c_len],
				     'wordpiece_mask': wordpiece_mask[:cur_bsz, :max_c_len]
			}
			yield data_dict
	...
	def get_test_batch(self):
		...
		for b in range(self.test_batches):
			...
			for i, index in enumerate(cur_batch):
				...
				wordpiece_map[i].copy_(torch.from_numpy(self.data_test_wordpiece_map[index, :, :]))
				wordpiece_mask[i].copy_(torch.from_numpy(self.data_test_wordpiece_mask[index, :, :]))
	...
	def train(self, model_pattern, model_name):
		...
		for epoch in range(self.max_epoch):
			...
			for ex, data in enumerate(self.get_train_batch()):
				...
				wordpiece_map = data['wordpiece_map']
				wordpiece_mask = data['wordpiece_mask']
				dis_h_2_t = ht_pair_pos+10
				dis_t_2_h = -ht_pair_pos+10

				predict_re = model(context_idxs, context_pos, context_ner, context_char_idxs, input_lengths, h_mapping, t_mapping, relation_mask, dis_h_2_t, dis_t_2_h, wordpiece_map, wordpiece_mask)
		...
		for data in self.get_test_batch():
			with torch.no_grad():
				...
				predict_re = model(context_idxs, context_pos, context_ner, context_char_idxs, input_lengths,
								   h_mapping, t_mapping, relation_mask, dis_h_2_t, dis_t_2_h, wordpiece_map, wordpiece_mask)
