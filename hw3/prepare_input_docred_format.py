  ...
  def load_data(self, data_file_name,
    ...
    whitespaces = {}
		...
		for i, d in enumerate(ori_data):
			...
			entities = []
			for ent_id in sorted(s.ent_ids, key=lambda x:int(x)):
				ent = {}
				for ner_id in s.ent_id_to_ner_id[ent_id]:
					...
					if not ne[2] in ent['names']:
						...
						ent['names'][ne[2]]['mentions'].append([ne[0], ne[1]]) 
		...
		json.dump(new_ds, open(os.path.join(self.out_path, self.split+'_new.json'), 'w'), indent=4, sort_keys=True)
