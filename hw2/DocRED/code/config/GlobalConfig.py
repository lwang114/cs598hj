...
class GlobalConfig(object):
	...
	def load_test_data(self):
		...
		vocabs = {'entity_type': json.load(os.path.join(self.data_path, 'ner2id.json')),
							'relation_type': self.rel2id}
		self.global_feature_maps = self.generate_global_feature_maps(vocabs)
	...
	def get_train_batch(self):
		...
		entity_label = torch.LongTensor(self.batch_size, self.entity_num).cuda()
		relation_triple = []
		...
		for b in range(self.train_batches):
			...
			for i, index in enumerate(cur_batch):
				...
				entities = ins['vertexSet']
				for j, entity in entities:
					entity_label[i, j] = self.vocabs['entity_type'][entity['type']]
				...
				for j, (h_idx, t_idx) in enumerate(train_tripe):
					...
					cur_relation_triple.append((h_idx, t_idx, label[rt]))
				relation_triple.append(cur_relation_triple)
			...
			yield {'context_idxs': context_idxs[:cur_bsz, :max_c_len].contiguous(),
				   ...
				   't_mapping': t_mapping[:cur_bsz, :max_h_t_cnt, :max_c_len],
					 'entity_label': entity_label[:cur_bsz, :max_h_t_cnt].contiguous(),
	...
	def get_test_batch(self):
		...
		ht_pair = []
		entity_label = torch.LongTensor(self.test_batch_size, self.entity_num).cuda()
		entity_label.fill_(IGNORE_INDEX)
					...
					for t_idx in range(L):
						if h_idx != t_idx:
							...
							if delta_dis < 0:
								...
								ht_pair_pos[i, j] = int(self.dis2idx[delta_dis])
							cur_ht_pair.append((h_idx, t_idx))
							j += 1
	...
	def train(self, model_pattern, model_name):
		...
		for epoch in range(self.max_epoch):
			...
			for data in self.get_train_batch():
				...
				relation_triple = data['relation_triple']
				...
				# global features
				B = predict_re.size(0) 
				graphs = [self.triple_to_graph(relation_triple[i], entity_label[i], predict_re[i]) for i in range(B)]
				gold_global_vector = [self.generate_global_feature_vector(graph) for graph in graphs]
				gold_scores = model.compute_graph_scores(graphs, predict_re, gold_global_vector) # TODO

				top_graphs = self.generate_locally_top_graphs(relation_triple, predict_re)
				top_global_vector = self.generate_global_feature_vector(top_graphs)
				top_scores = model.compute_graph_scores(top_graphs, predict_re, top_global_vector) # TODO
	...
	def test(self, model, model_name, output=False, input_theta=-1):
		...
		for data in self.get_test_batch():
			...
			entity_label = [[label for label in labels if label != IGNORE_INDEX] for labels in entity_label.data.cpu().numpy().tolist()] 
			batch_graphs = []
			for i in range(len(labels)):
				...
				entity_num = len(entity_label[i])
				if entity_num == 0:
					batch_graphs.append(Graph.empty_graph(self.vocabs))
					continue
				graph = self.decode(ht_pair, entity_label, relation_type_scores=predict_re) # Beam search
			...
			for i, graph in enumerate(batch_graphs):
				...
				L = L_vertex[i]
				j = 0
				for score, (h_idx, t_idx, r) in zip(graph.scores, graph.relations):
						...
						for r in range(1, self.relation_num):
							...
							test_result.append( ((h_idx, t_idx, r) in label, float(score), intrain,  titles[i], self.id2rel[r], index, h_idx, t_idx, r) )
	...
	def decode(self, ht_pair, entity_labels, scores, theta=0.5):
		""" Decode the optimal graph using beam search. 
		Adapted from OneIE: http://blender.cs.illinois.edu/software/oneie/
		"""	
		beam_set = [Graph.empty_graph(self.vocabs)] 
		beta_e = 10	
		beam_size = 100
		ht2idx = {(h, t): i for i, (h, t) in enumerate(ht_pair)}
		for h_idx, h_entity in enumerate(entity_labels): 
			for t_idx, t_entity in enumerate(entity_labels):
				if h_idx != t_idx:
					edge_scores = scores[ht2idx[(h_idx, t_idx)]]
					edge_scores = [(s, i) for i, s in enumerate(edge_scores)]
				edge_scores.sort(key=lambda x: x[0], reverse=True) # XXX Assume one label per edge
				top_edge_scores = edge_scores[:beta_e]

				beam_set_ = []
				for graph in beam_set:
					graph_ = graph.copy()
					for score, label in top_edge_scores:
						graph_.add_relation(h_idx, t_idx, label, score)
						beam_set_append(graph_)
				beam_set = beam_set_
				if len(beam_set) > 200:
	...
	def trim_beam_set(beam_set, beam_size):
		if len(beam_set) > beam_size:
			beam_set.sort(key=lambda x: self.compute_graph_score(x), reverse=True)
			beam_set = beam_set[:beam_size]
		return beam_set	

	def triple_to_graph(self, triples, entity_labels, scores):
		"""	
		:param triples (list): A list of relation triples of (h_idx, t_idx, r_idx) 
		:param entity_labels (torch.LongTensor): A tensor of entity labels
		:param scores (torch.FloatTensor): Score tensor with i-th row j-th column storing the label scores for the i-th triple to be of relation type j
		:return graph (object): Relation graph 
		"""
		entity_labels = [label for label in entity_labels.data.cpu().numpy().tolist() if label != IGNORE_INDEX]
		graph = Graph(entity_labels, [], [], self.vocabs)
		for triple, score in zip(triples, scores):
			h_idx = triple[0]
			t_idx = triple[1]
			r_idx = triple[2]
			graph.add_edge(h_idx, t_idx, r_idx, score[r_idx])
		return graph

	def generate_global_feature_mapping(self, vocabs):	
	...
	def generate_global_feature_vector(self, graph):
		""" Adapted from code for OneIE: http://blender.cs.illinois.edu/software/oneie/ """
		relation_entity_entity_map = self.global_feature_maps['relation_entity_entity']
		relation_entity_entity_vec = np.zeros(len(relation_entity_entity_map))
		relation_entity_map = self.global_feature_maps['relation_entity']
		relation_entity_vec = np.zeros(len(relation_entity_map))
		relation_role_role_map = self.global_feature_maps['relation_role_role']
		relation_role_role_vec = np.zeros(len(relation_role_role_map))
		multi_relation_map = self.global_feature_maps['multi_relation']
		multi_relation_vec = np.zeros(len(multi_relation_map))
		relation_relation_map = self.global_feature_maps['relation_relation']
		relation_relation_vec = np.zeros(len(relation_relation_map))
		entity_relation_count = [0 for _ in range(graph.entity_num)]
		for h_idx, t_idx, r_idx in graph.relations:
			entity_relation_count[h_idx][r_idx] += 1
			entity_relation_count[t_idx][r_idx] += 1
			h_entity = graph.entities[h_idx]			
			t_entity = graph.entities[t_idx]
			# 1. relation entity entity
			if h_entity < t_entity:
				key = r_idx * 10000 + h_entity * 100 + t_entity
			else:
				key = r_idx * 10000 + t_entity * 100 + h_entity
			if key in relation_entity_entity_map:
				relation_entity_entity_vec[relation_entity_entity_map[key]] += 1				
			# 2. relation entity
			key_h = r_idx * 100 + h_entity
			key_t = r_idx * 100 + t_entity
			if key_h in relation_entity_map:
				relation_entity_vec[relation_entity_map[key_h]] += 1
			if key_t in relation_entity_map:
				relation_entity_vec[relation_entity_map[key_t]] += 1
		
		# 3. multiple relation & 4. relation relation
		...
		feature_vector = np.concatenate([relation_entity_entity_vec, relation_entity_vec, multi_relation_vec, relation_relation_vec])
		return feature_vector

	def generate_locally_top_graphs(self, triples, entity_labels, scores):
		"""
		Adapted from code for OneIE: http://blender.cs.illinois.edu/software/oneie/
		:param triples (list): A list of relation triples of (h_idx, t_idx, r_idx)
		:param entity_labels (list): A list of entity labels
		:param scores (torch.FloatTensor): Score tensor with i-th row j-th column storing the label scores for the i-th triple to be of relation type j
		:return graphs (list): A list of graphs with the top local scores
		""" 
		top_graphs = []
		B = scores.size(0)
		scores = scores.data.cpu().numpy()
		entity_labels = [[label for label in labels if label != IGNORE_INDEX] for labels in entity_labels.data.cpu().numpy().tolist()] 
		for idx in range(B):
			top_relations = []
			top_scores = []
			for triple in triples[idx]:
				...
				for r_idx in range(self.relation_num):
					# Find the top score and label for each triple
					if score[idx][r_idx] >= 0.5:
						top_relations.append((h_idx, t_idx, r_idx))
						top_scores.append(score[idx][r_idx])
			top_graphs.append(Graph(entity_labels[idx], top_relations, top_scores, self.vocabs))
