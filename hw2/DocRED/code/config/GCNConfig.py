		...
		for b in range(self.train_batches):
			...
			for i, index in enumerate(cur_batch):
				...
				for t_idx, h_idx in enumerate(self.data_train_dephead[index]):
					if t_idx != h_idx:
						adj[min(h_idx+1, self.max_length-1), min(t_idx+1, self.max_length-1)] = 1. # XXX TEMP: +1 accounts for [CLS] symbol of the BERT tokenizer
				context_adj[i].copy_(torch.from_numpy(adj + np.eye(self.max_length))) # XXX Add identity matrix to introduce self-loop
		...
		for b in range(self.test_batches):
			...
			for i, index in enumerate(cur_batch):
				...
				for t_idx, h_idx in enumerate(self.data_test_dephead[index, :]):
					if t_idx != h_idx:
						adj[min(h_idx+1, self.max_length-1), min(t_idx+1, self.max_length-1)] = 1. # XXX TEMP: +1 accounts for [CLS] symbol of the BERT tokenizer 
				context_adj[i].copy_(torch.from_numpy(adj + np.eye(self.max_length)))
