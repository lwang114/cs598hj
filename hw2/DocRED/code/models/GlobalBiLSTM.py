...
class GlobalBiLSTM(nn.Module):
	def __init__(self, config):
		...
		self.global_feature_weights = nn.Parameter(
				torch.zeros(self.config.global_feature_num).fill_(-0.0001))
	...
	def compute_graph_score(self, graph, global_vector): # TODO
		...
		score = graph.graph_local_score + global_score
		return score

	def compute_graph_scores(self, graphs, scores, global_vectors):
		""" Adapted from OneIE: http://blender.cs.illinois.edu/software/oneie/
		:param graphs (list): A list of Graph objects
		:param global_vectors (tensor): A tensor of global vectors 
		:return score (tensor): Total score of the graphs
		"""
		max_relation_num = scores.size(1)
		relation_idxs = torch.LongTensor([graph.to_label_idxs(max_relation_num)[0] for graph in graphs])
		relation_masks = torch.LongTensor([graph.to_label_idxs(max_relation_num)[1] for graph in graphs])

		relation_idxs = relation_idxs.unsqueeze(-1)
		relation_scores = torch.gather(scores, 2, relation_idxs) * relation_mask
		relation_score = relation_scores.sum(1)

		global_vectors = relation_scores.new_tensor(global_vectors) # Create a tensor for the global vector
		...
		score = relation_score + global_score
		return score
